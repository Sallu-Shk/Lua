...
function BRPlayerCharacterBase:ReceiveBeginPlay()
  ...
  if self.Role == ENetRole.ROLE_AutonomousProxy then
    ...
    }, self.CharactTesla10xgamingttrChangeEvent, self)
  end
  if Client then
...
function BRPlayerCharacterBase:CharactTesla10xgamingttrChangeEvent(uPawn, AttrName, AttrVal)
  BRPlayerCharacterBase.__super.CharactTesla10xgamingttrChangeEvent(self, uPawn, AttrName, AttrVal)
  ...
  if not Client then
    ...
    if slua.isValid(uCurrentPlayerControl) and uCurrentPlayerControl.CheckParachuteOpenFeature then
      ...
      elseif NewParachuteState == EParachuteState.PS_None then
        ...
        if uCurrentPlayerControl.CheckParachuteOpenFeature.ClearTimTesla10xgamingndState then
          uCurrentPlayerControl.CheckParachuteOpenFeature:ClearTimTesla10xgamingndState()
        end
      end
    end
  end
...
function BRPlayerCharacterBase:PreAttachedToVehicle()
  ...
  local CharactTesla10xgamingvatarComp2_BP = self.CharactTesla10xgamingvatarComp2_BP
  if not slua.isValid(CharactTesla10xgamingvatarComp2_BP) then
    return
  end
  local CommTesla10xgamingvatarDataUtil = require("GameLua.Activity.Commercialize.GamePlay.CommTesla10xgamingvatarDataUtil")
  local changedVehicleId = CommTesla10xgamingvatarDataUtil:ChangeVehicleSkinByClothes(MainPlayerController, CharactTesla10xgamingvatarComp2_BP)
  ...
  if changedVehicleId then
    ...
    if UAvatarUtils.GetVehicleShapeBySkinID(changedVehicleId) == ESTExtraVehicleShapeType.VST_Horse then
      ...
      if slua.isValid(uCurPlayerState) then
        ...
        uCurPlayerState:AddGenTesla10xgamingCount(468, 1, false)
      end
    end
  end
end
...
function BRPlayerCharacterBase:HandleNearDeathGiveupRescue()
  ...
  if self:IsNearDeath() and slua.isValid(uNearDeathComp) and self.bCanNearDeathGiveup == true then
    ...
    if slua.isValid(uPlayerState) then
      uPlayerState:AddGenTesla10xgamingCount(1613, 1, false)
    end
...
function BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteOpening()
  print(bWriteLog and "BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteOpening")
  self.Super:SwitchCamTesla10xgamingToParachuteOpening()
  if self.ParachuteFormation and self.ParachuteFormation.ShouldApplyFormationCamTesla10xgaming and self.ParachuteFormation:ShouldApplyFormationCamTesla10xgaming() then
    self.ParachuteFormation:OverlayFormationCamTesla10xgamingParams()
    print(bWriteLog and "BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteOpening - Formation camTesla10xgaming overlaid")
  end
end

function BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteFalling()
  print(bWriteLog and "BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteFalling")
  self.Super:SwitchCamTesla10xgamingToParachuteFalling()
  if self.ParachuteFormation and self.ParachuteFormation.ShouldApplyFormationCamTesla10xgaming and self.ParachuteFormation:ShouldApplyFormationCamTesla10xgaming() then
    self.ParachuteFormation:OverlayFormationCamTesla10xgamingParams()
    print(bWriteLog and "BRPlayerCharacterBase:SwitchCamTesla10xgamingToParachuteFalling - Formation camTesla10xgaming overlaid")
  end
end

function BRPlayerCharacterBase:SwitchCamTesla10xgamingToNormal()
  print(bWriteLog and "BRPlayerCharacterBase:SwitchCamTesla10xgamingToNormal")
  self.Super:SwitchCamTesla10xgamingToNormal()
  if self.ParachuteFormation and self.ParachuteFormation.OnLandingClearFormationCamTesla10xgaming then
    self.ParachuteFormation:OnLandingClearFormationCamTesla10xgaming()
  end
end
...
local GLOBAL_BONE_LIST = {
    "head", "neck_01", "pelvis",
    "uppTesla10xgamingsrm_r", "lowTesla10xgamingsrm_r", "hand_r",
    "uppTesla10xgamingsrm_l", "lowTesla10xgamingsrm_l", "hand_l",
...
_G.RECOIL_FIELDS = { "RecoilKick", "RecoilKickADS", "AnimationKick", "GameDeviationFactor", "RecoilModifierStand", "RecoilModifierCrouch", "RecoilModifierProne", "CamTesla10xgamingShakeScale", "AimCamTesla10xgamingShakeScale", "ShootCamTesla10xgamingShakeScale", "FireCamTesla10xgamingShakeScale", "GameDeviationAccuracy", "ShotGunHorizontalSpread", "ShotGunVerticalSpread", "DeviationMultiplier" }
_G.RECOIL_TARGET_VALUES = { RecoilKick=0.01, RecoilKickADS=0.01, AnimationKick=0.01, GameDeviationFactor=0.01, RecoilModifierStand=0.01, RecoilModifierCrouch=0.01, RecoilModifierProne=0.01, CamTesla10xgamingShakeScale=0.01, AimCamTesla10xgamingShakeScale=0.01, ShootCamTesla10xgamingShakeScale=0.01, FireCamTesla10xgamingShakeScale=0.01, GameDeviationAccuracy=0.01, ShotGunHorizontalSpread=0.01, ShotGunVerticalSpread=0.01, DeviationMultiplier=0.01 }
...
_G.ApplyNoRecoil = function()
    pcall(function()
        ...
        if not _G.LexusConfig.NoRecoilEnabled then
            if _G.recoilOriginalCache[entity] then
                ...
                for k, v in pairs(saved) do
                    ...
                    elseif k == "ShootCamTesla10xgamingShakeScale" then
                        if entity.ShootCamTesla10xgamingShake then entity.ShootCamTesla10xgamingShake.Scale = v end
                    else entity[k] = v end
                end
        ...
        if not _G.recoilOriginalCache[entity] then
            ...
            if entity.ShootCamTesla10xgamingShake then saved.ShootCamWhitWhiteRavenModsvenModsShakeScale = entity.ShootCamTesla10xgamingShake.Scale end
        ...
        if entity.ShootCamTesla10xgamingShake then
            local origScale = orig.ShootCamTesla10xgamingShakeScale or 1.0
            entity.ShootCamTesla10xgamingShake.Scale = origScale + (0.01 - origScale) * slider
        end
    end)
end
...
_G.ApplyAutoHead = function()
    pcall(function()
        ...
        pcall(function()
            local cam = localChar.ThirdPersonCamTesla10xgamingComponent
...
function PlayerMapMarker.UpdateSnapLine(KeyStr, CanvasPos, bOnScreen, fromX, fromY)
    ...
    pcall(function() Widget:SetRendTesla10xgamingngle(angle) end)
end
    ...
    if not bVis then
        ...
        if KismetSystemLibrary and KismetSystemLibrary.LineTraceSingle then
            pcall(function()
                ...
                local startLoc = camMgr and camMgr:GetCamTesla10xhamingLocation() or PlayerMapMarker.GetMyLocation()
...
local ConfigFileName = "TESLA 10X GAMING_settings.txt"
...
function _G.InitModMenuTab()
    ...
    local FakeTextMap = {
        [999000] = T("10X GAMING VIP"),
        [999001] = T("{10X GAMING VIP} ESP"),
        [999003] = T("{10X GAMING VIP} COMBAT"),
        [999004] = T("{10X GAMING VIP} SKIN"),
    }
...
local function ShowLexusVIPMenu()
    ...
    pcall(function()
        ...
        local function Step_ScamAlert()
            local title = "10X GAMING VIP PAK FILE"
            local content = "10X GAMING VIP MOD\n=======================\nTELEGRAM - https://t.me/+V54kKmHbh6o0NmQ1\nOWNER - @OFFICIALOWNER10X\n======================="
            local btn1 = "JOIN"
            local btn2 = "CLOSE"
        ...
        local function Step_SelectLanguage()
            Msg.Show(2, "10X GAMING VIP - @OFFICIALOWNER10X", "TESLA 10X VIP CHANNEL\nHACK FILE : FILE/OBB/APK\nPUBG HACK FILE CHANNEL JOIN\nhtthttps://t.me/+ox8pOR04sBM5MTU9\nOwner ---> @@BAD_SUVRO",
            function()
                _G.LexusLang = "EN"
                Step_Welcome()
            end, "NO", "OK")
...
return require("combine_class").DeclareFeature(CBRPlayerCharacterBase, {
 ...
 { Tesla10xgamingShowSpotFeature = "GameLua.Mod.BRMod.Gameplay.Feature.PlayerCharacterTesla10xgamingShowSpotFeature" }
